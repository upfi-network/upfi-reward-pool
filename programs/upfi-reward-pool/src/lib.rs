...
declare_id!("CCurte7tnKYh1MBgZqR4cY3DixKSAZTfNg1AUHXSBLpm");

mod constants {
    pub const X_UPS_TOKEN_MINT_PUBKEY: &str = "DY98vGpLfSDs2hYLqiajYTLmeatk4kFyFv9GakQaKxna";
    pub const X_TOKEN_MINT_PUBKEY: &str = "t9Ab7CqA3W1qpgwPG1FzJLQdpf5K1ybeymAv2BjCbiu";
    pub const UPS_DEPOSIT_REQUIREMENT: u64 = 100_000_000_000_000;
...
pub mod upfi_pool {
    ...
    pub fn initialize_pool(
        ...
        token::transfer(cpi_ctx, constants::UPS_DEPOSIT_REQUIREMENT)?;
    ...
    pub fn unpause(ctx: Context<Unpause>) -> Result<()> {
        ...
        token::transfer(cpi_ctx, 1000_000_000_000)?;
    ...
    pub fn deposit(ctx: Context<Deposit>, nonce: u8, amount_a: u64, amount_b: u64) -> Result<()> {
        ...
        if amount_a > 0 {
            let cpi_ctx = CpiContext::new(
                ctx.accounts.token_program.to_account_info(),
                token::Transfer {
                    from: ctx.accounts.token_a_mint.to_account_info(),
        ...
        if amount_b > 0 {
            let cpi_ctx = CpiContext::new(
                ctx.accounts.token_program.to_account_info(),
                token::Transfer {
                    from: ctx.accounts.token_b_mint.to_account_info(),
        ...
        let wad_a: u64 = (amount_a as u128)
        .checked_mul(100)
        .unwrap()
        .checked_div(3)
        .unwrap()
        .try_into()
        .unwrap();
        let wad_b: u64 = (amount_b as u128)
        .checked_mul(100)
        .unwrap()
        .checked_div(3)
        .unwrap()
        .try_into()
        .unwrap();
        let wad: u64 = wad_a.checked_add(wad_b).unwrap().try_into().unwrap();
        ...
        if wad > 0 {
            let cpi_ctx = CpiContext::new_with_signer(
                ...
                token::MintTo {
                    ...
                    to: ctx.accounts.to_lp_token.to_account_info(),
                    authority: ctx.accounts.pool_signer.to_account_info(),
                },
                pool_signer,
            );
            token::mint_to(cpi_ctx, wad)?;
        }

        Ok(())
    }

    pub fn withdraw(ctx: Context<Withdraw>, nonce: u8, amount: u64) -> Result<()> {
        let seeds = &[
            ctx.accounts.pool.to_account_info().key.as_ref(),
            &[nonce],
        ];
        let pool_signer = &[&seeds[..]];
        let wad: u64 = (amount as u128)
        .checked_mul(3)
        .unwrap()
        .checked_div(200)
        .unwrap()
        ...
        // transfer upfi
        {
            let cpi_ctx = CpiContext::new_with_signer(
                ctx.accounts.token_program.to_account_info(),
                token::Transfer {
                    from: ctx.accounts.upfi_vault.to_account_info(),
                    to: ctx.accounts.to_upfi_account.to_account_info(),
                    authority: ctx.accounts.pool_signer.to_account_info(),
                },
                pool_signer,
            );
...
pub struct InitializePool<'info> {
    ...
    #[account(
        mut,
        constraint = x_token_pool_vault.mint == X_TOKEN_MINT_PUBKEY.parse::<Pubkey>().unwrap(),
        constraint = x_token_pool_vault.owner == pool_signer.key(),
    )]
    x_token_pool_vault: Box<Account<'info, TokenAccount>>,
    #[account(
        mut,
        constraint = x_token_depositor.mint == X_TOKEN_MINT_PUBKEY.parse::<Pubkey>().unwrap()
    )]
...
pub struct Deposit<'info> {
    #[account(
        mut,
        constraint = token_a_mint.mint == USDC_TOKEN_MINT_PUBKEY.parse::<Pubkey>().unwrap()
    )]
    token_a_mint: Box<Account<'info, TokenAccount>>,
    #[account(
        mut,
        constraint = token_a_vault.mint == USDC_TOKEN_MINT_PUBKEY.parse::<Pubkey>().unwrap()
    )]
    token_a_vault: Box<Account<'info, TokenAccount>>,
    #[account(
        mut,
        constraint = token_b_mint.mint == UPFI_TOKEN_MINT_PUBKEY.parse::<Pubkey>().unwrap()
    )]
    token_b_mint: Box<Account<'info, TokenAccount>>,
    #[account(
        mut,
        constraint = token_b_vault.mint == UPFI_TOKEN_MINT_PUBKEY.parse::<Pubkey>().unwrap()
    )]
    token_b_vault: Box<Account<'info, TokenAccount>>,
    #[account(
        mut,
        address = constants::X_UPS_TOKEN_MINT_PUBKEY.parse::<Pubkey>().unwrap(),
    )]
    lp_token_mint: Box<Account<'info, Mint>>,

    #[account(
        mut,
        constraint = to_lp_token.mint == X_UPS_TOKEN_MINT_PUBKEY.parse::<Pubkey>().unwrap()
    )]
    to_lp_token: Box<Account<'info, TokenAccount>>,
...
pub struct Withdraw<'info> {
    ...
    #[account(
        mut,
        constraint = to_usdc_account.mint == USDC_TOKEN_MINT_PUBKEY.parse::<Pubkey>().unwrap()
    )]
    to_usdc_account: Box<Account<'info, TokenAccount>>,
    #[account(
        mut,
        constraint = usdc_vault.mint == USDC_TOKEN_MINT_PUBKEY.parse::<Pubkey>().unwrap()
    )]
    usdc_vault: Box<Account<'info, TokenAccount>>,
    #[account(
        mut,
        constraint = to_upfi_account.mint == UPFI_TOKEN_MINT_PUBKEY.parse::<Pubkey>().unwrap()
    )]
    to_upfi_account: Box<Account<'info, TokenAccount>>,
    #[account(
        mut,
        constraint = upfi_vault.mint == UPFI_TOKEN_MINT_PUBKEY.parse::<Pubkey>().unwrap()
    )]
    upfi_vault: Box<Account<'info, TokenAccount>>,
